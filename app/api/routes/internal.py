...
def check_job_key(x_job_key: str | None = Header(None)):
    if not JOB_KEY or x_job_key != JOB_KEY:
        raise HTTPException(status_code=401, detail="invalid job key")

@router.get("/entries/{entry_id}")
def get_entry(entry_id: int, _: None = Depends(check_job_key), db: Session = Depends(get_db)):
...
def patch_sentiment(entry_id: int, body: dict, _: None = Depends(check_job_key), db: Session = Depends(get_db)):
